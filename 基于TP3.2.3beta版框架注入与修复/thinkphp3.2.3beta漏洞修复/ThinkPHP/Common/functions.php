...
if (!empty($_COOKIE)){
    if(is_array($_COOKIE)){
        array_walk_recursive($_COOKIE,'think_filter');
    }
}
...
function U($url='',$vars='',$suffix=true,$domain=true) {
    ...
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
        ...
        if($router_ruler && C('URL_Rewrite')){
        	//$info   =  parse_url($url);
        	$urlkey = '';
        	if(isset($vars['_addons'])){
        		$urlkey = 'addon/'.$vars['_addons'].'/'.$vars['_controller'].'/'.$vars['_action'];
        	}else{
        		$urlkey = implode($depr,array_reverse($var));
        	}
        	//路由命中
        	if (isset($router_ruler[$urlkey])) {
        		//$url = '/'.$router_ruler[$url];
        		//$url = str_ireplace($urlkey,$router_ruler[$urlkey], $url);
        		$url = __ROOT__.'/'.$router_ruler[$urlkey];
        		unset($vars['_addons']);
 ...
 */
function is_ssl() {
	if (isset($_SERVER['HTTP_VIA']) && $_SERVER['HTTP_VIA'] == 'nginx-ssl') {
		return true;
	}
